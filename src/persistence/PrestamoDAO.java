...
public class PrestamoDAO {
    ...
    public List<Prestamo> obtenerTodos() throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                Date sqlDevolucion = rs.getDate("fecha_devolucion");
    ...
    public List<Prestamo> obtenerPrestamosActivosPorCarnet(String carnet) throws SQLException {
        ...
        String sql = "SELECT p.id, d.titulo AS nombre_documento, p.fecha_prestamo, p.fecha_devolucion, u.carnet " +
                "FROM prestamos p " +
                "JOIN usuarios u ON p.id_usuario = u.id " +
                "JOIN documentos d ON p.id_documento = d.id " +
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Date sqlDevolucion = rs.getDate("fecha_devolucion");
                    if (sqlDevolucion != null) {
                        p.setFechaDevolucion(sqlDevolucion.toLocalDate());
                    } else {
